...
void __print_sensor(struct SensorData sd) {
...
void __print_packet(struct DecodedPacket dp) {
...
void __fill_in_sensordata(struct SensorData *sd, int16_t *data) {
	sd->gyro_x = data[0];
	sd->gyro_y = data[1];
	sd->gyro_z = data[2];
	sd->acc_x = data[3];
...
struct DecodedPacket __decode_packet(uint8_t *data) {
